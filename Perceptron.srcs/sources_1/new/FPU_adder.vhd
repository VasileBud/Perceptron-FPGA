...
architecture mixed of FPU_adder is
...
--      done    <= '0';
      ...
      case state is
        when WAIT_STATE =>
          if (start = '1') then
            A_sgn      <= A(31);
            A_exp      <= '0' & A(30 downto 23);	--bit for signed subtraction
            A_mantissa <= "01" & A(22 downto 0);	--Two bits extra, one for leading 1 and other one for storing carry
            B_sgn      <= B(31);
        ...
        when ALIGN_STATE =>  --Compare and align exponent
          --If any is greater than 2**24 skip
